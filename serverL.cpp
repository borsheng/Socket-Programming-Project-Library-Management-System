...
std::map<std::string, int> literatureBooks;
...
constexpr int SERVER_S_PORT = 42162;
...
void readBooksFromInput() {
    ...
    while (std::getline(file, line)) {
        ...
        literatureBooks[code] = count;
    }
}

void updateBookInventory(const std::string& code) {
    if (literatureBooks[code] > 0) {
        literatureBooks[code]--;
    }
}
...
int main() {
    ...
    // initialize server
    std::cout << "Server L is up and running using UDP on port " << SERVER_S_PORT <<"." << std::endl;
    ...
    server_addr.sin_port = htons(42162);
    ...
    while (true) {
        ...
        buffer[recv_len] = '\0';
        std::cout << "Server L received " << buffer << " code from the Main Server." << std::endl;

        std::string response;
        if (literatureBooks.find(buffer) != literatureBooks.end()) {
            // numbrer of books avalible
            int count = literatureBooks[buffer];
            if (count > 0) {
        ...
        if (sent_bytes < 0) {
            ...
            std::cout << "Server L finished sending the availability status of code " << buffer << " to the Main Server using UDP on port " << ntohs(server_addr.sin_port) << "." << std::endl;
        }
